...
func renderType2(numberOfCol: Int, cellSizeWidth: Int, currentRow: Int, gridArray: [[GameObject?]], lineIndex: Int) -> String {
    ...
    for colIndex in 0..<numberOfCol {
        ...
        if gridArray[currentRow][colIndex] != nil {
...
func printGrid(cellSizeWidth: Int, cellSizeLength: Int, world: TankWorld) {
    let gridArray = world.grid
    let numberOfRow = world.numberRows
    let numberOfCol = world.numberCols
    var grid = ""
    grid += renderType1(numberOfCol: numberOfCol, cellSizeWidth: cellSizeWidth)
    for rowIndex in 0..<numberOfRow {
        grid += renderRow(numberOfCol: numberOfCol, cellSizeWidth: cellSizeWidth, cellSizeLength: cellSizeLength, gridArray: gridArray, currentRow: rowIndex)
    }
    print(grid)
}
